...
const handleClear = async (ctx) => {
    try {
        ...
        await ctx.reply("🗑️ ¡Podemos empezar de cero!");
    }
    catch (error) {
...
bot.on(["message:voice", "message:audio"], async (ctx) => {
    ...
    try {
        ...
        // Download the file using grammy's built-in method for simplicity and robustness
...
async function handleBotMessage(ctx, text) {
...
app.post('/api/chat', async (req, res) => {
    try {
        const { message, project_id, skill } = req.body;
        // For web interface, we use a fixed 'web' chat_id
        const activeSkill = skill || await getSetting('web', "active_skill") || 'personal';
        const reply = await processUserMessage(message, 'web', project_id, activeSkill);
        res.json({ reply });
    }
    catch (err) {
