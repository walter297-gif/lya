...
const MESSAGES_TO_KEEP_PER_CHAT = 50;
...
try {
    ...
    `);
    const pruneRes = prune.run(MESSAGES_TO_KEEP_PER_CHAT);
    console.log(`[Cleanup] 🗑️  Eliminados ${pruneRes.changes} mensajes antiguos (se conservaron los últimos ${MESSAGES_TO_KEEP_PER_CHAT} por chat).`);
