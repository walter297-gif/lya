...
import { getHistory, saveMessage, getSetting, setSetting } from '../db/index.js';
...
const sqlCache = new SimpleLRUCache<string, any>(50);
...
async function getSystemPrompt(skill: string): Promise<string> {
...
export async function processUserMessage(userContent: string, chat_id: string = 'default', project_id?: string, skill: string = 'personal'): Promise<string> {
    ...
    let iterations = 0;
    const baseSystemPrompt = await getSystemPrompt(skill);
    const contextSummary = await getSetting(chat_id, "chat_summary") || "Nueva conversación.";
    const activeProduct = await getSetting(chat_id, "active_product") || "Ninguno.";

    // Inyectar estado dinámico en el prompt del sistema
    const systemPrompt = `${baseSystemPrompt}\n\n--- 🧠 MEMORIA DE TRABAJO (ESTADO ACTUAL) ---\nResumen de la Charla: ${contextSummary}\nProducto Seleccionado actualmente: ${activeProduct}\n-------------------------------------------`;
    ...
    while (iterations < MAX_ITERATIONS) {
        ...
        try {
            ...
            // Resumen automático si el historial crece demasiado (Optativo/Heurístico)
            if (history.length > 20 && iterations === 1) {
                console.log("[Loop] History too long, should summarize in next cycle.");
            }
            ...
            if (response.tool_calls && response.tool_calls.length > 0) {
                ...
                for (const toolCall of response.tool_calls) {
                    ...
                    try {
                        ...
                        if (functionName === 'execute_sql_query') {
                            ...
                            if (typeof args === 'string') {
                                ...
                                if (args.sql) args.query = args.sql;
                                else if (args.consulta) args.query = args.consulta;
                                else if (Object.keys(args).length === 1 && typeof Object.values(args)[0] === 'string') {
                                    const val = Object.values(args)[0] as string;
                    ...
                    try {
                        ...
                        // PERSISTENCIA DE ESTADO: Si la consulta SQL fue exitosa, guardamos el código del producto
                        if (functionName === 'execute_sql_query' && args.query && !result.error) {
                             // Intentar extraer el código del SP call: EXEC [dbo].[Sp_Busca_Control_de_Stock] 'L1', 'CODIGO'
                             const codeMatch = args.query.match(/'([^']+)'\s*,\s*'([^']+)'/); // Busca el segundo parámetro
                             const codeValue = codeMatch ? codeMatch[2] : null;
                             if (codeValue && codeValue !== 'L1') {
                                 console.log(`[Loop State] Persisting active product: ${codeValue}`);
                                 await setSetting(chat_id, "active_product", codeValue);
                             }
                        }
                    } catch (error) {
                ...
                // ACTUALIZAR RESUMEN (Post-Respuesta) para la próxima vez
                if (history.length > 8) {
                    await updateConversationSummary(chat_id, history);
                }
...
// Función encargada de mantener la memoria de largo plazo condensada
async function updateConversationSummary(chat_id: string, history: LLMMessage[]): Promise<void> {
    console.log(`[Summary] Generating new summary for chat ${chat_id}...`);

    const summaryPrompt: LLMMessage[] = [
        { 
            role: 'system', 
            content: 'Eres un experto en gestión de memoria de IA. Resume la intención del usuario y la situación actual de la charla en máximo 2 líneas. No menciones herramientas técnicas, solo el flujo del negocio (ej: "El usuario busca rentabilizar el producto X").' 
        },
        ...history.slice(-8), // Tomar los últimos mensajes relevantes
        { role: 'user', content: 'Resume el estado de esta charla brevemente:' }
    ];

    try {
        const response = await generateResponse(summaryPrompt);
        if (response.content) {
            await setSetting(chat_id, "chat_summary", response.content.trim());
            console.log(`[Summary] New Summary: ${response.content.trim()}`);
        }
    } catch (e) {
        console.warn("[Summary Error] Failed to update:", e);
    }
}
